...
class OrdersController extends Controller
{
    ...
    public function getShow(Request $request){
    	...
    	$data = DB::table('orders')->join('address','address.id','=','orders.address_id')->join('goods','goods.id','=','orders.goodsid')->select('orders.*','goods.gname','address.name','address.phone','address.sheng1','address.shi1','address.xian1','address.jiedao1','address.xiangxi')->skip($page)->take($num)->where('recycle',0)->where('order_num','like','%'.$key.'%')->orderby('orders.addtime','desc')->groupBy('order_num')->groupBy('uid')->get();
    ...
    public function postDel(Request $request){
        ...
        $uid=$request->input('uid'); //用户id
        $res=DB::table('orders')->where('uid',$uid)->where('order_num',$id)->update(['recycle'=>1]);
        if($res){
            echo 'yes';
        }else{
            echo 'no';
